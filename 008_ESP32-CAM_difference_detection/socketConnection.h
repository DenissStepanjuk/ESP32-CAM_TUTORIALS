  ...
  switch (type) {
    ...
    case WStype_TEXT:   // Если клиент отправил текстовые данные, обработать их.
      ...
      // При успешной записи JSON строки в память контролера обработать её.
        ...
        if(String(msg_type) == "capture_main") {
          // Присвоим bool capture_main значение true чтобы в void loop() {} 
          // выполнить блок кода отвечающий за захват изображения.
          capture_main = doc_rx["value"];
        }

        if(String(msg_type) == "capture_current") {
          // Присвоим bool capture_current значение true чтобы в void loop() {} 
          // выполнить блок кода отвечающий за захват изображения.
          capture_current = doc_rx["value"];
        }
      }
      Serial.println("");
      break;
