  ...
  switch (type) {
    ...
    case WStype_TEXT:   // Если клиент отправил текстовые данные, обработать их.
      ...
      // При успешной записи JSON строки в память контролера обработать её.
        ...
        }else if(String(msg_type) == "hue_minimum") {
          sendJson(jsonString, doc_tx, String(msg_type), doc_rx["value"]);
          hue_min = doc_rx["value"];
          Serial.println("hue_minimum: " + String(doc_rx["value"]));

        } else if(String(msg_type) == "hue_maximum") {
          sendJson(jsonString, doc_tx, String(msg_type), doc_rx["value"]);
          hue_max = doc_rx["value"];
          Serial.println("hue_maximum: " + String(doc_rx["value"]));

        } else if(String(msg_type) == "saturation_minimum") {
          sendJson(jsonString, doc_tx, String(msg_type), doc_rx["value"]);
          sat_min = doc_rx["value"];
          Serial.println("saturation_minimum: " + String(doc_rx["value"]));

        } else if(String(msg_type) == "saturation_maximum") {
          sendJson(jsonString, doc_tx, String(msg_type), doc_rx["value"]);
          sat_max = doc_rx["value"];
          Serial.println("saturation_maximum: " + String(doc_rx["value"]));

        } else if(String(msg_type) == "value_minimum") {
          sendJson(jsonString, doc_tx, String(msg_type), doc_rx["value"]);
          val_min = doc_rx["value"];
          Serial.println("value_minimum: " + String(doc_rx["value"]));

        } else if(String(msg_type) == "value_maximum") {
          sendJson(jsonString, doc_tx, String(msg_type), doc_rx["value"]);
          val_max = doc_rx["value"];
          Serial.println("value_maximum: " + String(doc_rx["value"]));
        } 
      }
      Serial.println("");
      break;
