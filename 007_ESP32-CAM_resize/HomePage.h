...
"        };"
"      }"
    // Переменная должна укзывать на то, какое изображение принимаем  (imgFULL или imgBIG или imgSMALL).
"   var img_type = 0;"
...
"       } else if(img_type == 1){"
"         document.getElementById('imgBIG').src = URL.createObjectURL(event.data);"
"       } else if(img_type == 2){"
"         document.getElementById('imgSMALL').src = URL.createObjectURL(event.data);"
"       }"
"			} else {"
"				  try {"
