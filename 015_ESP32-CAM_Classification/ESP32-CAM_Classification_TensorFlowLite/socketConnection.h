  ...
  switch (type) {
    ...
    case WStype_TEXT:   // Если клиент отправил текстовые данные, обработать их.
      ...
      // При успешной записи JSON строки в память контролера обработать её.
        ...
        if(String(msg_type) == "classify") {
          // Присвоим bool classify значение true чтобы в void loop() {} 
          // выполнить блок кода отвечающий за захват изображения и его классификацию.
          classify = doc_rx["value"];
        }

        if(String(msg_type) == "dataset") {
          // Присвоим bool dataset значение true чтобы в void loop() {} 
          // выполнить блок кода отвечающий за захват изображения и его запись на карту памяти.
